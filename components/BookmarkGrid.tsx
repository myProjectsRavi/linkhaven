 ...
 * - Memoized BookmarkCard component prevents unnecessary re-renders
 * - useCallback for stable function references
 * - useMemo for computed values
 * 
 * For lists > 50 items, consider adding react-window virtualization.
 * Current implementation handles 1,000+ items smoothly with memoization.
 */
...
// Individual bookmark card component (memoized for performance)
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {bookmark.snapshot ? (
          ...
          <button
            ...
            disabled={isSaving}
            className={`p-1.5 rounded ${isSaving
        ...
        <button
          onClick={(e) => {
            e.stopPropagation();
            if (confirm("Delete this bookmark?")) onDelete();
          }}
...
export const BookmarkGrid: React.FC<BookmarkGridProps> = ({
  ...
  const handleSaveSnapshot = useCallback(async (bookmark: Bookmark) => {
